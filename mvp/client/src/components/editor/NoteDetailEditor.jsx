...
function NoteDetailEditor({
  ...
  const resolvedOwner = note.owner || '未指定';

  const dirty = useMemo(() => {
    return (
      title !== lastSavedSnapshot.title ||
      summary !== lastSavedSnapshot.summary ||
      highlightsText !== lastSavedSnapshot.highlightsText ||
      body !== lastSavedSnapshot.body
    );
  }, [title, summary, highlightsText, body, lastSavedSnapshot]);
  ...
  }, [dirty, title, resolvedOwner, summary, highlightsText, body, onSave, note.highlights, note.folderPath, note.tags, note.id]);
